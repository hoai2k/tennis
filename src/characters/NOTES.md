...
- **Humanoid mech re-rigs** (`*_rig.glb`, the "MECH RIGS" setting, ON by default → `modelUrl()` in `core/roster.ts`): titanus/konga/fenrir/vulcan ship a second model with the standard Rigify DEF-* skeleton and no clips, driven by the procedural Animator like the rest of the roster. Per-character `FLAVORS` in `animator.ts` layer personality onto the shared states (konga: hunched knuckle stance + chest-beat victory; titanus: wide ponderous stance, slow stride, stomp victory; fenrir: feral lean, loping stride; vulcan: squared gun-platform bearing) — stance applies only to grounded states so swing-contact timing stays exact. saurion/nullbot/frogger `*_rig` exports are BROKEN at the source: their DEF skeleton is displaced from the mesh in bind space (the weights point at a miniature skeleton at the model's feet — verified S = boneWorld·invBind is one uniform mini-scale+offset per model), so posing them smears the mesh and no load-time transform can move the pivots. They stay on their clip rigs (see `HUMANOID_RIG_READY`) until re-exported with the armature transform applied; their flavors are already authored.
